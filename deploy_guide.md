...
项目提供了自动化部署脚本，存放在 `scripts/` 目录下。

#### 脚本列表

| 脚本 | 用途 |
|------|------|
| `scripts/config.sh` | 配置文件（服务器路径、数据库信息等） |
...
维护模式下，前台显示维护页面（HTTP 503），Admin 后台仍可正常访问。

```bash
# 开启维护模式
ssh aliyun "touch /var/www/ai_blog/maintenance/.maintenance && nginx -s reload"

# 关闭维护模式
ssh aliyun "rm -f /var/www/ai_blog/maintenance/.maintenance && nginx -s reload"

# 查看状态
ssh aliyun "ls /var/www/ai_blog/maintenance/.maintenance 2>/dev/null && echo '维护模式: 开启' || echo '维护模式: 关闭'"
```
